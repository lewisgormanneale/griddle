...
export async function getNonogram(id: number): Promise<Tables<'nonograms'> | undefined> {
...
export async function getPacks({
  ...
  page: number;
  pageSize: number;
}): Promise<{ data: Tables<'packs'>[]; count: number }> {
  const supabase = createClient();
...
export async function getNonogramsForPack(id: number): Promise<Tables<'nonograms'>[]> {
  const supabase = createClient();
  try {
    const { data, error } = await supabase.from('nonograms').select('*').eq('pack_id', id);
    if (error) {
      throw new Error(error.message);
    }
    return data;
