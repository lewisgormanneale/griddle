...
export function Hints({nonogram, grid, isColumn}: {
    ...
    useEffect(() => {
        ...
        const newHints = Array.from({length}, (_, index) => {
            ...
            for (let i = 0; i < (isColumn ? nonogram.rows : nonogram.columns); i++) {
                line.push(solutionArray[isColumn ? i * nonogram.columns + index : index * nonogram.columns + i]);
            }
            return line.join("").split("0").filter(hint => hint !== "").map(hint => hint.length);
        });
        setHints(newHints);
    }, [nonogram, isColumn]);
